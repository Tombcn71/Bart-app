...
type HorType = "inzethor" | "klemhor" | "plisse-hordeur";

const TABS: { key: HorType; label: string; matrixKey: string }[] = [
  { key: "inzethor",       label: "Inzethor",       matrixKey: "inzethor_matrix" },
  { key: "klemhor",        label: "Klemhor",         matrixKey: "klemhor_matrix" },
  { key: "plisse-hordeur", label: "Plisse hordeur",  matrixKey: "plisse_hordeur_matrix" },
];

export default function HorrenPrijzen() {
  const [activeTab, setActiveTab] = useState<HorType>("inzethor");
  const [data, setData] = useState<Record<HorType, any>>({
    "inzethor": null, "klemhor": null, "plisse-hordeur": null,
  });
  ...
  useEffect(() => {
    async function load() {
      ...
      const newData = {} as Record<HorType, any>;
      TABS.forEach((t, i) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-black text-slate-800">Horren Prijslijst</h1>
          <p className="text-slate-500 text-sm">Stel per hortype de basisprijs en toeslagen in.</p>
        </div>
