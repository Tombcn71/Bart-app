...
export default function HarmonicadeurPrijzenBeheer() {
  ...
  useEffect(() => {
    async function loadData() {
      const data = await getMatrix("harmonicadeur_matrix");
      setPrijzen(data && Object.keys(data).length ? data : DEFAULT_MATRIX);
      setLoading(false);
    }
    loadData();
  }, []);
  ...
  const handleSave = async () => {
    if (!prijzen) return;
    setSaving(true);
    await saveMatrix("harmonicadeur_matrix", prijzen);
    setSaving(false);
    alert("Harmonicadeur prijzen succesvol opgeslagen!");
  };
  ...
  const handleChange = (key: string, value: any, subKey?: string) => {
    if (subKey) {
      setPrijzen((prev: any) => ({
        ...prev,
        [key]: { ...prev[key], [subKey]: parseFloat(value) || 0 },
      }));
    } else {
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-black text-slate-800">
            Harmonicadeur Prijslijst
          </h1>
          <p className="text-slate-500 text-sm">
            Beheer hier de prijzen voor de harmonicadeur configurator.
          </p>
        </div>
        <button
          onClick={handleSave}
