// Pure string-based SVG generatie voor rolluiken (geen React, werkt in server actions).

const C  = "#000000";
const LW = 0.8;
const LT = 0.5;
const LS = 0.4;   // lamellen
const DC = "#9ca3af"; // subtiele maat-kleur
const DS = 0.4;

function r(n: number) { return n.toFixed(1); }
function sRect(x: number, y: number, w: number, h: number, fill: string, stroke: string, sw: number) {
  return `<rect x="${r(x)}" y="${r(y)}" width="${r(w)}" height="${r(h)}" fill="${fill}" stroke="${stroke}" stroke-width="${sw}"/>`;
}
function sLine(x1: number, y1: number, x2: number, y2: number, stroke: string, sw: number) {
  return `<line x1="${r(x1)}" y1="${r(y1)}" x2="${r(x2)}" y2="${r(y2)}" stroke="${stroke}" stroke-width="${sw}"/>`;
}
function sPoly(points: string, fill: string) { return `<polygon points="${points}" fill="${fill}"/>`; }
function sText(x: number, y: number, txt: string, anchor: string, fs: number, fill: string, transform = "") {
  return `<text x="${r(x)}" y="${r(y)}" text-anchor="${anchor}" font-family="Arial,sans-serif" font-size="${fs}" fill="${fill}" ${transform}>${txt}</text>`;
}

function lamellen(x: number, y: number, w: number, h: number, step: number): string {
  let s = "";
  for (let dy = step; dy < h; dy += step) s += sLine(x, y+dy, x+w, y+dy, C, LS);
  return s;
}

function dimH(x1: number, x2: number, y: number, label: string) {
  const mx = (x1+x2)/2;
  return sLine(x1,y+3,x1,y-3,DC,DS)+sLine(x2,y+3,x2,y-3,DC,DS)+sLine(x1,y,x2,y,DC,DS)
    +sPoly(`${r(x1)},${r(y)} ${r(x1+5)},${r(y-2)} ${r(x1+5)},${r(y+2)}`,DC)
    +sPoly(`${r(x2)},${r(y)} ${r(x2-5)},${r(y-2)} ${r(x2-5)},${r(y+2)}`,DC)
    +sRect(mx-16,y-5,32,10,"white","none",0)+sText(mx,y+3.5,label,"middle",11,DC);
}
function dimV(y1: number, y2: number, x: number, label: string) {
  const my = (y1+y2)/2;
  return sLine(x-3,y1,x+3,y1,DC,DS)+sLine(x-3,y2,x+3,y2,DC,DS)+sLine(x,y1,x,y2,DC,DS)
    +sPoly(`${r(x)},${r(y1)} ${r(x-2)},${r(y1+5)} ${r(x+2.5)},${r(y1+7)}`,DC)
    +sPoly(`${r(x)},${r(y2)} ${r(x-2)},${r(y2-5)} ${r(x+2.5)},${r(y2-7)}`,DC)
    +sRect(x-5,my-16,10,32,"white","none",0)
    +sText(x,my+3.5,label,"middle",11,DC,`transform="rotate(-90,${r(x)},${r(my)})"`);
}

export function buildRolluikSvgString(slug: string, breedte: number, hoogte: number): string {
  const vW = 100, vH = 100;
  const hasKast = slug === "rolluik" || slug === "screen";
  const step = slug === "screen" ? 4 : 7;

  let body = "";
  if (hasKast) {
    body += sRect(5, 5, 90, 12, "white", C, LW);
    body += sRect(9, 7, 82,  8, "none",  C, LT);
    body += sRect(5, 17, 90, 78, "white", C, LW);
    body += sRect(9, 21, 82, 70, "none",  C, LT);
    body += lamellen(9, 21, 82, 70, step);
  } else {
    body += sRect(5, 5, 90, 90, "white", C, LW);
    body += sRect(9, 9, 82, 82, "none",  C, LT);
    body += lamellen(9, 9, 82, 82, step);
  }

  const PL = 30, PT = 8, PB = 24, PR = 6;
  const totalW = vW + PL + PR, totalH = vH + PT + PB;
  const wY = PT + vH + 15, hX = PL - 18;

  const witnesses =
    sLine(PL,    PT+vH+3, PL,    wY-2, DC, DS)+sLine(PL+vW,PT+vH+3,PL+vW,wY-2,DC,DS)+
    sLine(PL-3,  PT,      hX+2,  PT,   DC, DS)+sLine(PL-3, PT+vH,  hX+2, PT+vH,DC,DS);

  const dims = dimH(PL,PL+vW,wY,`${breedte} mm`)+dimV(PT,PT+vH,hX,`${hoogte} mm`);

  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${totalW} ${totalH}" width="${totalW}" height="${totalH}">
  <rect x="0" y="0" width="${totalW}" height="${totalH}" fill="white"/>
  <g transform="translate(${PL},${PT})">${body}</g>
  ${witnesses}${dims}
</svg>`;
}
